...
echo ""
echo "First I need to know the new password of MySQL debian user:"
...
#mysql -u debian -p
so2=$(expect -c "
spawn mysql -u debian -p; sleep 3
expect \"\";  sleep 3; send \"$DatabasePass\r\"
...
#rm -f /debian/jcameron-key.asc

#rm -R /home/vps/public_html/installation

cd
rm -f /debian/.bash_history && history -c
...
echo "" | tee -a log-install.txt
echo "Installation Log --> /debian/log-install.txt" | tee -a log-install.txt
echo "=======================================================" | tee -a log-install.txt
cd ~/

#rm -f /debian/ocspanel.sh
